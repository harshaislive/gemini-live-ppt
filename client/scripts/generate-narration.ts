...
import { NARRATION_CHUNKS } from "../app/presentationScript";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const CLIENT_ROOT = path.resolve(__dirname, "..");
const REPO_ROOT = path.resolve(CLIENT_ROOT, "..");
const OUT_DIR = path.join(CLIENT_ROOT, "public", "audio", "narration");
...
async function main() {
  ...
  for (const chunk of NARRATION_CHUNKS) {
    ...
    if (!data) {
      throw new Error(`No audio returned for ${chunk.id}`);
    }
